...
if(arraySize > rangeDifference){
    ...
    for(let j = arraySize-1; j >= 0 ;j--){
        let randomIndex = Math.floor(Math.random()*(j-0));
        console.log(randomIndex)
        let temp = availableElemArray[j];
        availableElemArray[j] = availableElemArray[randomIndex];
        availableElemArray[randomIndex] = temp;
        //console.log("inside availableElemArray",availableElemArray);
        randomArray[j] = availableElemArray[j];
        maxRange--;
    }
...
if(givenDuplicatePercent < minDuplicatePercent){
    ...
    let fetchLastElementIndex = Math.round(Math.random()*(0.5));
